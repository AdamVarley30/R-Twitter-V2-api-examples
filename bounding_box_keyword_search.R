...
keyword_spatial_search <- function(db_name,start_date,end_date,spatial_query,keyword_query,bbox,tweets_per_request){
  ...
  # I have changed this to spatial searches there should be relatively few matches with the bbox
  ...
  for(i in nrow(time_frame):1){
    ...
    while(!is.null(next_token)){
      ...
      ,error = function(err) {
        message(time_frame$from[i] ,'dropped out',err)
...
# Plot coordinates to check if they are right
plot(search_grid,col = 'grey')

# Query 
keyword_query = '(football goal) lang:en'
i = 1
